...
const ManageAllProduct = () => {
    ...
    const deleteAproduct = id => {
        if (lipsticks.length > 12) {
            ...
            if (confirm) {
                const url = `https://nameless-citadel-84200.herokuapp.com/lipsticks/${id}`
                fetch(url, {
                    method: 'DELETE',
                })
                    .then(res => res.json())
                    .then(data => {
                        if (data.deletedCount > 0) {
                            ...
                            const remainingProducts = lipsticks.filter(remaining => remaining._id !== id);
    ...
    return (
        <div style={{ marginBottom: "50px" }}>
            ...
            <Box sx={{ mb: 5 }}>
                <Typography variant="h4" sx={{ textAlign: 'center', mt: 2 }}>You can manage the products</Typography>
                <Container>
                    <Grid container spacing={3} sx={{ my: 2 }}>
                        {
                            remainingProducts.map(products => <Products
                                key={products._id}
                                products={products}
...
function Products({ products, deleteAproduct }) {
    ...
    return (
        <Grid item xs={12} sm={12} md={6} sx={{ p: 0 }}>
            <Paper elevation={0} sx={{ backgroundColor: '#540D15', textAlign: 'center', display: 'flex', alignItems: 'center', justifyContent: 'space-between', border: 3, borderRadius: '10px' }}>
                <img style={{ overflow: 'hidden', borderRadius: '10px' }} width="40%" height="200px" src={img} alt="" />
                <Box sx={{ textAlign: 'start', ml: 1, width: '60%', color: 'white' }}>
                    ...
                    <Button onClick={() => deleteAproduct(_id)} sx={{ backgroundColor: 'white', color: 'goldenrod', fontWeight: 'bold' }}>Delete</Button>
                </Box>
            </Paper>
        </Grid>
